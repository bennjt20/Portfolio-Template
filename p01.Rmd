...
```{r Df to CSV}
library(readr)
write_csv(cats, "~/Documents/GitHub/Portfolio-Template/p01_files/cats.csv") 
```
...
```{r read in image file, Photo in directory}
cat1.png <- readPNG("~/Documents/GitHub/Portfolio-Template/p01_files/cat1.png")
catbox.png <- readPNG("~/Documents/GitHub/Portfolio-Template/p01_files/catbox.png")
```
...
ggplot(data = cats, aes(x = age, y = incidents)) +
  annotation_custom(rasterGrob(catbox.png, 
                               width = unit(1,"npc"),
                               height = unit(1,"npc")), 
                    -Inf, Inf, -Inf, Inf) +
  geom_point() +
...
7) Next, we modify the code for the graph to include our data point photo which is now labeled "image1", for the column in our dataframe. 
